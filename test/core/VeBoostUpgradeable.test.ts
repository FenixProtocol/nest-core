...
describe('VeBoostUpgradeable', function () {
  ...
  async function deployPriceProviderWith(
    ...
    usdReserve: bigint,
    fnxReserve: bigint,
  ): Promise<AlgebraNESTPriceProviderUpgradeable> {
  ...
  describe('Deployment', function () {
    ...
    it('Should corect set initial parameters', async function () {
      expect(await veBoost.nest()).to.be.equal(await fenix.getAddress());
    ...
    it('Should correct initialzie initial parameters', async () => {
      ...
      expect(await veBoost.getMinNESTAmountForBoost()).to.be.closeTo(ethers.parseEther('10'), ethers.parseEther('10') / BigInt(10000));
      expect(await veBoost.getMinLockedTimeForBoost()).to.be.equal(182 * 86400);
      expect(await veBoost.getBoostNESTPercentage()).to.be.equal(1000);
  ...
  describe('Ownable functions', async () => {
    ...
    describe('#setNESTBoostPercentage', async () => {
      it('should fail if try call from not owner', async () => {
        await expect(veBoost.connect(signers.otherUser1).setNESTBoostPercentage(50)).to.be.revertedWith(ERRORS.Ownable.NotOwner);
      });
      it('should correct call and set parameters', async () => {
        expect(await veBoost.getBoostNESTPercentage()).to.be.eq(1_000);
        await expect(veBoost.setNESTBoostPercentage(5678)).to.be.emit(veBoost, 'NESTBoostPercentage').withArgs(5678);
        expect(await veBoost.getBoostNESTPercentage()).to.be.eq(5678);
      });
    });
    describe('#setMinUSDAmount', async () => {
      ...
      it('should correct call and set parameters', async () => {
        ...
        expect(await veBoost.getMinNESTAmountForBoost()).to.be.closeTo(
    ...
    describe('#addRewardToken', async () => {
      ...
      it('should fail if try add fenix like reward token', async () => {
        await expect(veBoost.addRewardToken(await veBoost.nest())).to.be.revertedWithCustomError(veBoost, 'RewardTokenExist');
      });
  ...
  describe('View functions', async () => {
    ...
    describe('getMinNESTAmountForBoost', async () => {
      ...
      it('Should return current min fnx amount correctly after update', async () => {
        ...
        expect(await veBoost.getMinNESTAmountForBoost()).to.be.closeTo(ethers.parseEther('100'), ethers.parseEther('100') / BigInt(10000));
      });
      it('Should return current min fnx amount correctly after update price', async () => {
        ...
        expect(await veBoost.getMinNESTAmountForBoost()).to.be.closeTo(ethers.parseEther('20'), ethers.parseEther('20') / BigInt(10000));
    ...
    describe('#calculateBoostNESTAmount', async () => {
      it('Should return zero fnx amount less the min FNX amount', async () => {
        expect(await veBoost.calculateBoostNESTAmount(ZERO)).to.be.eq(ZERO);
        expect(await veBoost.calculateBoostNESTAmount(ONE)).to.be.eq(ZERO);
        expect(await veBoost.calculateBoostNESTAmount(ethers.parseEther('9.99') - ONE)).to.be.eq(ZERO);
      });
      it('Should corect calculate boost fnx amount', async () => {
        let minFNXAmount = ethers.parseEther('50');
        expect(await veBoost.calculateBoostNESTAmount(minFNXAmount)).to.be.eq(ethers.parseEther('5')); // 10%
        expect(await veBoost.calculateBoostNESTAmount(ethers.parseEther('50.1'))).to.be.eq(ethers.parseEther('5.01')); // 10%
        expect(await veBoost.calculateBoostNESTAmount(ethers.parseEther('1234567.123456'))).to.be.eq(ethers.parseEther('123456.7123456')); // 10%
      });
      it('Should corect calculate boost fnx amount after change boostPercentage', async () => {
        let minFNXAmount = ethers.parseEther('50');
        expect(await veBoost.calculateBoostNESTAmount(minFNXAmount)).to.be.eq(ethers.parseEther('5')); // 10%

        await veBoost.setNESTBoostPercentage(523); // 5.23%
        expect(await veBoost.calculateBoostNESTAmount(ethers.parseEther('50'))).to.be.eq(ethers.parseEther('2.615')); // 10%

        await veBoost.setNESTBoostPercentage(0); // 5.23%
        expect(await veBoost.calculateBoostNESTAmount(ethers.parseEther('50'))).to.be.eq(ZERO); // 0%

        await veBoost.setNESTBoostPercentage(10001); // 100.01%
        expect(await veBoost.calculateBoostNESTAmount(ethers.parseEther('50'))).to.be.eq(ethers.parseEther('50.005')); // 100.01%
      });
    });
  });
  describe('#beforeNESTBoostPaid', async () => {
    it('fail if caller not votingEscrow', async () => {
      await expect(
        veBoost
          .connect(signers.otherUser1)
          .beforeNESTBoostPaid(signers.otherUser1.address, 1, ethers.parseEther('100'), ethers.parseEther('10')),
    ...
    it('fail if provide not expected paidBoostFNXAmount', async () => {
      await expect(
        veBoost
          ...
          .beforeNESTBoostPaid(signers.otherUser1.address, 1, ethers.parseEther('49'), ethers.parseEther('1')),
      ).to.be.revertedWithCustomError(veBoost, 'InvalidBoostAmount');

      await expect(
        veBoost
          .connect(mockVotingEscrow)
          .beforeNESTBoostPaid(signers.otherUser1.address, 1, ethers.parseEther('50'), ethers.parseEther('5') + ONE),
      ).to.be.revertedWithCustomError(veBoost, 'InvalidBoostAmount');
    });
    ...
    describe('success -', async () => {
      ...
      describe('call with one additional token distribution', async () => {
        ...
        it('should correct work with tokens 6 decimlas', async () => {
          ...
          await veBoost
            .connect(mockVotingEscrow)
            .beforeNESTBoostPaid(signers.otherUser1.address, 1, ethers.parseEther('990'), ethers.parseEther('99'));
  ...
  describe('tests cases - ', async () => {
    for (let index = 0; index < Cases.length; index++) {
      ...
      it(`${index} case - ${eToString(iterator)}`, async () => {
        ...
        await veBoost.setNESTBoostPercentage(iterator.fnxBoostPercentage);

        let paidBoost = await veBoost.calculateBoostNESTAmount(iterator.depositedFNXAmount || 0);

        paidBoost = paidBoost > (await veBoost.getAvailableBoostNESTAmount()) ? await veBoost.getAvailableBoostNESTAmount() : paidBoost;

        await veBoost.connect(mockVotingEscrow).beforeNESTBoostPaid(signers.otherUser1.address, 1, iterator.depositedFNXAmount, paidBoost);
