...
describe('GaugesRewardsDistribution', function () {
  ...
  async function createPermanentVeNFT(user: HardhatEthersSigner, amount: bigint): Promise<bigint> {
    await fenix.connect(user).approve(votingEscrow.target, amount);
    await votingEscrow.connect(user).createLockFor(amount, 0, user.address, false, true, 0);
    return await votingEscrow.lastMintedTokenId();
  }
  ...
  async function advanceToNextEpochAndUpdate(skipDistributionWindow: boolean = true): Promise<bigint> {
    await advanceToNextEpoch(skipDistributionWindow);
    await deployed.minter.update_period();
    return await voter.epochTimestamp();
  }

  async function createStableVotingHistory(votingEpochsCount: number, finalEpochAfterDistributionWindow: boolean) {
    await setupFullEnvironment();

    const tokenId = await createPermanentVeNFT(signers.otherUser1, ethers.parseEther('1'));
    await voter.connect(signers.otherUser1).vote(tokenId, [v2Pool1.target], [10000]);

    const epochs = [await voter.epochTimestamp()];
    const weight = await voter.weightsPerEpoch(epochs[0], v2Pool1.target);
    expect(weight).to.be.eq(ethers.parseEther('1'));
    expect(await voter.totalWeightsPerEpoch(epochs[0])).to.be.eq(weight);

    for (let i = 1; i < votingEpochsCount; i++) {
      const epoch = await advanceToNextEpochAndUpdate(true);
      await voter.connect(signers.otherUser1).poke(tokenId);
      epochs.push(epoch);
      expect(await voter.weightsPerEpoch(epoch, v2Pool1.target)).to.be.eq(weight);
      expect(await voter.totalWeightsPerEpoch(epoch)).to.be.eq(weight);
    }

    const currentEpoch = await advanceToNextEpochAndUpdate(finalEpochAfterDistributionWindow);
  ...
  async function getDistributionState(gauge: GaugeUpgradeable) {
    const gaugeState = await voter.gaugesState(gauge.target);
    return {
      gaugeBalance: await fenix.balanceOf(gauge.target),
      voterBalance: await fenix.balanceOf(voter.target),
      claimable: gaugeState.claimable,
      gaugeIndex: gaugeState.index,
      lastDistributionTimestamp: gaugeState.lastDistributionTimestamp,
      rewardRate: await gauge.rewardRate(),
      periodFinish: await gauge.periodFinish(),
    };
  }

  async function checkSeededDistribution(params: {
    gauge: GaugeUpgradeable;
    indexEpochs: bigint[];
    indexes: bigint[];
    expectedReward: bigint;
    expectedCurrentEpoch: bigint;
    expectedLastDistributionBefore: bigint;
  }) {
    const before = await getDistributionState(params.gauge);
    expect(before.gaugeBalance).to.be.eq(0);
    expect(before.claimable).to.be.eq(0);
    expect(before.gaugeIndex).to.be.eq(0);
    expect(before.lastDistributionTimestamp).to.be.eq(params.expectedLastDistributionBefore);
    expect(before.rewardRate).to.be.eq(0);
    expect(before.periodFinish).to.be.eq(0);

    await fenix.transfer(voter.target, params.expectedReward);
    const voterBalanceAfterFunding = await fenix.balanceOf(voter.target);

    await voter.reinitialize(params.indexEpochs, params.indexes);
    for (let i = 0; i < params.indexEpochs.length; i++) {
      expect(await voter.indexPerEpoch(params.indexEpochs[i])).to.be.eq(params.indexes[i]);
    }

    const tx = await (voter as any).distribute([params.gauge.target]);
    const after = await getDistributionState(params.gauge);
    const expectedRewardRate = params.expectedReward / BigInt(WEEK);

    await expect(tx).to.be.emit(voter, 'DistributeReward').withArgs(signers.deployer.address, params.gauge.target, params.expectedReward);
    expect(after.gaugeBalance - before.gaugeBalance).to.be.eq(params.expectedReward);
    expect(voterBalanceAfterFunding - after.voterBalance).to.be.eq(params.expectedReward);
    expect(after.claimable).to.be.eq(0);
    expect(after.lastDistributionTimestamp).to.be.eq(params.expectedCurrentEpoch);
    expect(after.rewardRate).to.be.eq(expectedRewardRate);
    expect(await params.gauge.rewardForDuration()).to.be.eq(expectedRewardRate * BigInt(WEEK));
    expect(after.periodFinish).to.be.gt(before.periodFinish);
  }

  describe('Zero indexPerEpoch distribution regressions', function () {
    const INDEX_100M = ethers.parseEther('100000000');
    const INDEX_200M = ethers.parseEther('200000000');

    it('distributes across a single zero index epoch without breaking reward calculation', async () => {
      const { epochs, currentEpoch, weight, gauge } = await createStableVotingHistory(3, false);
      const expectedReward = (weight * INDEX_200M) / ethers.parseEther('1');

      await checkSeededDistribution({
        gauge,
        indexEpochs: [epochs[0] + BigInt(WEEK), epochs[1] + BigInt(WEEK), epochs[2] + BigInt(WEEK)],
        indexes: [INDEX_100M, 0n, INDEX_200M],
        expectedReward,
        expectedCurrentEpoch: currentEpoch,
        expectedLastDistributionBefore: epochs[0],
      });
    });

    it('distributes after several leading zero index epochs', async () => {
      const { epochs, currentEpoch, weight, gauge } = await createStableVotingHistory(4, false);
      const expectedReward = (weight * INDEX_100M) / ethers.parseEther('1');

      await checkSeededDistribution({
        gauge,
        indexEpochs: [epochs[0] + BigInt(WEEK), epochs[1] + BigInt(WEEK), epochs[2] + BigInt(WEEK), epochs[3] + BigInt(WEEK)],
        indexes: [0n, 0n, 0n, INDEX_100M],
        expectedReward,
        expectedCurrentEpoch: currentEpoch,
        expectedLastDistributionBefore: epochs[0],
      });
    ...
    it('distributes the same catch-up rewards after the distribution window has passed', async () => {
      const { epochs, currentEpoch, weight, gauge } = await createStableVotingHistory(2, true);
      const expectedReward = (weight * INDEX_200M) / ethers.parseEther('1');

      expect(BigInt(await time.latest())).to.be.gt(currentEpoch + BigInt(await voter.distributionWindowDuration()));

      await checkSeededDistribution({
        gauge,
        indexEpochs: [epochs[0] + BigInt(WEEK), epochs[1] + BigInt(WEEK)],
        indexes: [INDEX_100M, INDEX_200M],
        expectedReward,
        expectedCurrentEpoch: currentEpoch,
        expectedLastDistributionBefore: epochs[0],
      });
