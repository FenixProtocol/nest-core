...
describe('PairFactoryUpgradeable Contract', function () {
  ...
  describe('Deployments', async () => {
    ...
    it('fails if try initialize second time', async () => {
      await expect(pairFactory.initialize(ZERO_ADDRESS, feesVaultFactory.target)).to.be.revertedWith(ERRORS.Initializable.Initialized);
    });
    ...
    it('fails if provide zero fees vault factory', async () => {
      ...
      await expect(attached.initialize(deployed.v2PairImplementation.target, ZERO_ADDRESS)).to.be.revertedWithCustomError(
        pairFactoryFactory,
        'AddressZero',
      );
    });
  ...
  describe('Fees', async () => {
    ...
    describe('#setFee', async () => {
      ...
      it('fails if try set protocol fee > MAX_FEE% or fee == 0', async () => {
        await expect(pairFactory.setFee(false, 501)).to.be.revertedWithCustomError(pairFactory, 'IncorrectFee');
        await expect(pairFactory.setFee(false, 0)).to.be.revertedWithCustomError(pairFactory, 'IncorrectFee');
        await expect(pairFactory.setFee(true, 501)).to.be.revertedWithCustomError(pairFactory, 'IncorrectFee');
        await expect(pairFactory.setFee(true, 0)).to.be.revertedWithCustomError(pairFactory, 'IncorrectFee');
      });
  ...
  describe('#createPair', async () => {
    ...
    it('fails if try initialzie created Pair second time', async () => {
      ...
      await expect(p.initialize(tokenTK18.target, tokenTK6.target, true, deployed.feesVaultFactory.target)).to.be.rejectedWith(
        'Initialized',
      );
    });
