...
async function main() {
  ...
  {
    let VeNestSplitMerklAidropUpgradeable_Proxy = await ethers.getContractAt(InstanceName.VeNestSplitMerklAidropUpgradeable, DeployedContracts[AliasDeployedContracts.VeNestSplitMerklAidropUpgradeable_Proxy]);
    await logTx(VeNestSplitMerklAidropUpgradeable_Proxy, VeNestSplitMerklAidropUpgradeable_Proxy.transferOwnership(TARGET_OWNERSHIP));
  }
